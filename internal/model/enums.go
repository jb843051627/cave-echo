...
func (s SurveyStage) CanMoveTo(next SurveyStage) bool {
	...
	switch s {
	case SurveyPlanned:
		return next == SurveyFieldwork || next == SurveyReview
	case SurveyFieldwork:
		return next == SurveyReview || next == SurveyPlanned
	case SurveyReview:
		return next == SurveyClosed || next == SurveyFieldwork || next == SurveyPlanned
	default:
		return false
	}
}
