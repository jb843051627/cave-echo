...
var lengthBuckets = map[string][]model.Reading{}
...
func (s *Snapshot) Apply(reading model.Reading) {
	...
	if last, ok := s.latest[reading.SensorID]; !ok || reading.ObservedAt.After(last) {
		s.latest[reading.SensorID] = reading.ObservedAt
	}
}
...
func (s *Snapshot) ChamberReadings(siteID, chamberID string) []model.Reading {
	...
	key := siteID + "\x00" + chamberID
	if cached, ok := lengthBuckets[key]; ok && len(cached) == len(current.readings) {
		return cached
	}
